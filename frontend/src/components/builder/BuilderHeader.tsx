          ...
          <div className="relative w-full max-w-lg bg-white rounded-2xl shadow-2xl border border-slate-200 overflow-hidden animate-in zoom-in-95 duration-200 flex flex-col">
            ...
            {emailStep === 'compose' && (
              <form onSubmit={handleSendEmail} className="p-6 space-y-4">
                ...
                {emailError && (
                  <div className="p-4 text-sm bg-amber-50/80 border border-amber-200 rounded-xl space-y-3">
                    <div className="flex items-start gap-2.5 text-amber-900 font-medium">
                      <AlertCircle className="w-5 h-5 text-amber-600 shrink-0 mt-0.5" />
                      <div className="space-y-1">
                        <p className="font-bold text-amber-950">Email Configuration Required</p>
                        <p className="text-xs text-amber-800 leading-relaxed">{emailError}</p>
                      </div>
                    </div>

                    {(emailError.includes('Google Workspace') || emailError.includes('not connected')) && (
                      <div className="flex flex-col sm:flex-row gap-2 pt-1">
                        ...
                        <a
                          ...
                          target="_blank"
                          rel="noreferrer"
                          className="flex items-center justify-center gap-1.5 px-3.5 py-2 bg-slate-900 border border-slate-800 rounded-lg text-xs font-bold text-white hover:bg-slate-800 transition-all cursor-pointer text-center"
                        >
                          <Settings size={14} /> Configure SMTP Settings
                        </a>
                      </div>
                    )}
                  </div>
